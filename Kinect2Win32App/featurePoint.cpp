...
void featurePoint::getCircle200(IplImage *srcBw, vector<Point> &contours){
	for (int j = 0; j < contours.size(); j += contours.size() / pointinterval + 1){
		cvCircle(srcBw, contours[j], 3, CV_RGB(255, 0, 0), -1, 8, 0);
...
void featurePoint::getSpecialPoint27(IplImage *srcBw, vector<Point>&contoursPoint){
